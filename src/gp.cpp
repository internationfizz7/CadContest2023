...
void GP::RandomPosition(NETLIST & tmpnt )
{
    srand( time(NULL) );
    for( int i = 0; i < tmpnt.nMod; i++ )
    {
        //if(tmpnt.mods[i].is_m==true){
        tmpnt.mods[i].GCenterX = tmpnt.mods[i].modW/2+rand()%(tmpnt.ChipWidth- tmpnt.mods[i].modW);
        tmpnt.mods[i].GCenterY = tmpnt.mods[i].modH/2+rand()%(tmpnt.ChipHeight- tmpnt.mods[i].modH);
...
void GP::CreateQmBv( vector<QP>& matrixQ, float* vectorB, NETLIST& tmpnt, int modeXY, map<int, int>& mapNet, map<string, int>& mapMod )
{
    ...
    for( int i = 0; i < nNet; i++ )
    {
        ...
        if(tmpnt.nets[netID].flagAlign)                     ///< for alignment (2019.2.13)
            pinWeight*=10;
        ...
        for( int j = tmpnt.nets[netID].head; j < tmpnt.nets[netID].head + tmpnt.nets[netID].nPin; j++ )
        {
            ...
            {
                if( modeXY == 0 )
                    vectorB[i] += pinWeight * (float)tmpnt.pads[ tmpnt.pins[j].index-tmpnt.nMod ].x/AMP_PARA/2;
                else
                    vectorB[i] += pinWeight * (float)tmpnt.pads[ tmpnt.pins[j].index-tmpnt.nMod ].y/AMP_PARA/2;
            }
...
{
    for(int i=0;i<tmpnt.nMod;i++){
        cout<<tmpnt.mods[i].GCenterX<<" "<<tmpnt.mods[i].GCenterY<<"\n";
    }
