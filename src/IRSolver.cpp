...
void IRSOLVER::IRmain(IRBIN& bin, NETLIST& nt)
{
    ...
    ///construct layer
    int TSVnum=0;
    Layers.reserve(nt.nLayer*4+2);
    Nodes.reserve(nt.nLayer*4+2);
    ConstructLayer(*NameToMaterial["PCB"], "PCB", 0, 0, 1);

    for(int i=0; i<nt.nLayer; i++)
    {
...
void IRSOLVER::IRmain(vector< vector<IRBIN> >& bin_matrix, NETLIST& nt)
{
    ...
    for(unsigned int i=0; i<bin_matrix.size(); i++)
    {
        for(unsigned int j=0; j<bin_matrix[i].size(); j++)
        {
            ...
            TSVNumVector.assign(nt.nLayer, 0);
            PowerSumVector.assign(nt.nLayer, 0.0);
