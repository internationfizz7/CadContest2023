...
class GP 
{
	...
	protected:
		...
		double Fk_0;						///< objective function f_k-1
		double Fk_1;						///< objective function f_k
		...
		double lambda;						///< lambda in objective function
		double lambdaW;						///< Wirelength Weight
		double *alpha;						///< step size
		double overflow_area_0;				///< sum of max(Db(x,y)-Mb) in each bin (former)
		double overflow_area_1;				///< sum of max(Db(x,y)-Mb) in each bin (current)
		...
		class BIN
		{
			public:
				int idBin;					///< bin id
				int x;						///< left-bottom x-coor
				int y;						///< left-bottom y-coor
				int w;						///< width
				int h;						///< height
