...
Tree flutes_RDP(int d, DTYPE xs[], DTYPE ys[], int s[], int acc)
{
    ...
    return flutes_ALLD(d, xs, ys, s, acc); // when call "flutes_LD: has segmentation fault
}
...
Tree flutes_LD(int d, DTYPE xs[], DTYPE ys[], int s[])
{
    ...
    t.deg = d;
    t.branch = (Branch *) malloc((3*d-2)*sizeof(Branch));
    if (d == 2) {
        ...
        // ERROR 2020.02.21 // rlist->seg[0] seg. fault
        for (i=0; rlist->seg[i]>0; i++){
